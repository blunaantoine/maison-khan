...
import { isGoogleConfigured, getRedirectUri, signSession, SESSION_COOKIE, SESSION_MAX_AGE, OAUTH_STATE_COOKIE } from '@/lib/google-auth'
 ...
 *  5. Pose d'un cookie de session signé (HMAC), puis redirection vers /
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 5. Session signée + redirection vers l'accueil (le front restaure la session)
    ...
    // L'état OAuth est à usage unique
    response.cookies.set(OAUTH_STATE_COOKIE, '', { httpOnly: true, path: '/', maxAge: 0 })
    return response
  } catch (error) {
